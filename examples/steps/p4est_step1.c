...
refine_fn(p4est_t *p4est, p4est_topidx_t which_tree, p4est_quadrant_t *quadrant) {
    int           tilelen;      // each quadrant covers tilelen x tilelen pixels
    int           offsi, offsj; // pixel offset of the quadrant
    int           i, j;         // loop variables
    const char   *d;            // pointer to the pixel data
    unsigned char p[3];         // pixel color value (RGB)
   ...
   * We can then visualize the output and highlight level > PATTERN_LEVEL. */
    ...
    for (j = 0; j < tilelen; ++j) {
      ...
      d = hw32_header_data + 4 * (ple * (ple - 1 - (offsj + j)) + (offsi + i));
#else
...
int main(int argc, char **argv) {
    ...
    * from processor zero.  Here we turn off most of the logging; see sc.h. */
    ...
    P4EST_GLOBAL_PRODUCTIONF("This is the p4est %dD demo example/steps/%s_step1\n", P4EST_DIM, P4EST_STRING);
    ...
    * checked to execute dimension-dependent code. */
    ...
    p4est_vtk_write_file(p4est, NULL, P4EST_STRING "_new");
    ...
    * We check against the data dimensions in example/steps/hw32.h. */
    ...
    p4est_vtk_write_file(p4est, NULL, P4EST_STRING "_refine");
    ...
    * are possibly ready for coarsening, are never split between processors. */
    ...
    p4est_vtk_write_file(p4est, NULL, P4EST_STRING "_partition");
    ...
    * neighbors across edges or corners as well; see p4est.h. */
    ...
    p4est_vtk_write_file(p4est, NULL, P4EST_STRING "_balance");

    /* Write the forest to disk for visualization, one file per processor. */
    p4est_vtk_write_file(p4est, NULL, P4EST_STRING "_end");
