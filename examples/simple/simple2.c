...
int main(int argc, char **argv) {
    int                        mpiret;                           // mpi return value
    int                        wrongusage;                       // flag for wrong usage
    unsigned                   crc;                              // crc checksum of the forest
    const char                *usage;                            // usage message
    mpi_context_t              mpi_context, *mpi = &mpi_context; // mpi context
    p4est_t                   *p4est;                            // p4est data structure
    p4est_connectivity_t      *connectivity;                     // connectivity data structure
    p4est_geometry_t          *geom;                             // geometry data structure
    p4est_refine_t             refine_fn;                        // refinement function
    p4est_coarsen_t            coarsen_fn;                       // coarsening function
    simple_config_t            config;                           // configuration type
    const simple_regression_t *r;                                // regression data structure
    int                        nbrick_x = 1, nbrick_y = 1;       // number of bricks in x and y direction
